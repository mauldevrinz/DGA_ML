...
impl<'a> UsbComm<'a> {
    ...
    fn poll_setpoint(&mut self, rx_buf: &mut [u8], rx_len: &mut usize) -> Option<f32> {
        self.poll();
            ...
            if b == b'\n' || b == b'\r' {
                if *rx_len > 0 {
                    if let Ok(line) = core::str::from_utf8(&rx_buf[..*rx_len]) {
                        if let Some(rest) = line.trim().strip_prefix("SETPOINT=") {
...
fn main() -> ! {
    ...
    //   suhu > setpoint + hysteresis  -> mode COOL (polaritas "kiri"/LPWM)
    //   suhu < setpoint - hysteresis  -> mode OFF
    //   di antara keduanya            -> pertahankan mode sebelumnya
    //                                    (anti-chattering)
    //
    // Kalau ternyata Peltier malah memanas saat mode COOL aktif, berarti
    // polaritas kabel Peltier ke OUT1/OUT2 BTS7960 terbalik dari asumsi -
    // cukup ganti tanda di baris `set_peltier_power(-PELTIER_COOL_POWER_PERCENT)`
    // di bawah jadi positif, tidak perlu bongkar wiring.
    // ===================================================
    ...
    loop {
        for gpio_idx in 0..6 {
            ...
            // NYALAKAN GPIO AKTIF
            match gpio_idx {
                0 => { let _ = p15.set_high(); }
                1 => { let _ = p14.set_high(); }
                2 => { let _ = p41.set_high(); }
                3 => { let _ = p40.set_high(); }
                4 => { let _ = p39.set_high(); }
                5 => { let _ = p38.set_high(); }
                _ => {}
            }
