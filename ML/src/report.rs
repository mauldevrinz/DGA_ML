...
/// `caption`: optional sub-title drawn above the grid. Pass `None` when the
/// caller already prints its own section heading (avoids a duplicate label).
fn confusion_matrix_grid(l: &PdfLayerReference,
                         f: &IndirectFontRef, fb: &IndirectFontRef,
                         matrix: [[usize; 2]; 2], caption: Option<&str>, y: f32) -> f32 {
