//! MLP LOOO Training GUI - Pure Rust Implementation (No Gnuplot)
...
#[allow(dead_code)]
    ...
    fn start_training(&mut self) {
        ...
        thread::spawn(move || {
            ...
            println!(
                "{}",
                "║      Coffee Arabica MLP Classifier           ║"
                    .bold()
                    .cyan()
            );
            println!(
                "{}",
                "║    MLP Leave-One-Origin-Out Testing    ║"
                    .bold()
                    .cyan()
            );
            println!(
            ...
            if let Ok(rd) = std::fs::read_dir("models") {
                for entry in rd.flatten() {
                    ...
                    if name.starts_with("loo_mlp_curve_") && name.ends_with(".csv") {
            ...
            println!("\n{}", "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━".cyan());
            println!("{}", " Phase 2/4: MLP on TSFRESH features".bold().cyan());
            println!("{}", "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━".cyan());

            // MLP pakai fitur TSFRESH RAW (MLP melakukan z-score sendiri di dalam fit).
            let train_mlp = &split.train_raw;
            let val_mlp = &split.test_raw;
            ...
            println!("   Arsitektur: {} -> {} -> {} -> 2", n_features, mlp_config.hidden1, mlp_config.hidden2);

            let training_start = Instant::now();
            let mut model = CoffeeMLP::new(mlp_config);
            let live = tx.clone();
            let (_fit_result, accuracy_curve, loss_curve) = match model.fit_with_curve_features(
                train_mlp,
                &train_labels,
                val_mlp,
                &val_labels,
                |done, total, train_loss, train_acc, val_loss, val_acc| {
                    // Kirim metrics ke GUI untuk grafik live (epoch end)
            ...
            {
                use std::io::Write;
                let curve_path = format!("models/loo_mlp_curve_{}.csv", fold_idx);
            ...
            std::fs::create_dir_all("models").ok();
            model.save("models/loo_mlp_model.json").ok();

            } // ── akhir loop 12 fold ──

            // ── OPSI 3: Latih MLP FULL pakai SEMUA 12 origin (untuk prediksi) ──
            // MLP menormalkan fitur secara internal -> tidak perlu file norm terpisah.
            {
                println!("\n🔧 Melatih model MLP FULL (12 origin) untuk prediksi...");
                let full_cfg = MLPConfig {
                    hidden1: 128, hidden2: 64, n_epochs: 100,
                    learning_rate: 0.01, batch_size: 16, clip_grad: 5.0,
                    disable_early_stop: true,
                };
                let mut full_model = CoffeeMLP::new(full_cfg);
                let noop = |_:usize,_:usize,_:f32,_:f32,_:f32,_:f32| {};
                match full_model.fit_features(&dataset.features, &dataset.labels, &dataset.features, &dataset.labels, noop) {
                    Ok(_) => {
                        std::fs::create_dir_all("models").ok();
                        full_model.save("models/loo_mlp_full_model.json").ok();
                        println!("✅ Model MLP FULL tersimpan (loo_mlp_full_model.json).");
                    }
                    Err(e) => eprintln!("❌ Gagal latih MLP FULL: {}", e),
                }
            }
            ...
            {
                use std::io::Write;
                if let Ok(mut f) = std::fs::File::create("models/loo_mlp_predictions.csv") {
                    let _ = writeln!(f, "p_high,label");
                    for (s, l) in all_scores.iter().zip(all_labels.iter()) {
                        let _ = writeln!(f, "{:.6},{}", s, l);
                    }
                }
                if let Ok(mut f) = std::fs::File::create("models/loo_mlp_folds.csv") {
...
impl eframe::App for TrainingGUI {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("header")
            .exact_height(60.0)
            .frame(egui::Frame::none().fill(egui::Color32::from_rgb(0, 130, 110)))
            .show(ctx, |ui| {
                ui.vertical_centered(|ui| {
                    ui.add_space(15.0);
                    ui.label(egui::RichText::new("Leave-One-Origin-Out for MLP")
                        .size(28.0)
                    ...
                    if let Some(ref pm) = self.power_monitor {
                        if let Some(s) = pm.current_sample() {
                            ...
                            ui.horizontal(|ui| {
                                ...
                                .size(13.0)
                                .color(egui::Color32::from_rgb(0, 130, 110))
                                .family(egui::FontFamily::Name("Poppins".into())));
                            });
                            ui.add_space(6.0);
                        }
            ...
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    ...
                    if pred_resp.clicked() {
                        launch_predict_binary("predict_loo_mlp_gui");
                    }
...
impl TrainingGUI {
    fn save_pdf(&mut self) {
        ...
        if let Ok(content) = std::fs::read_to_string("models/loo_mlp_folds.csv") {
        ...
        match generate_loo_summary_pdf("MLP", &folds, &loo_roc, &agg_eval, total_secs, "testing_results") {
    ...
    fn build_agg_eval() -> EvaluationResults {
        ...
        if let Ok(content) = std::fs::read_to_string("models/loo_mlp_predictions.csv") {
            for (li, line) in content.lines().enumerate() {
                if li == 0 || line.trim().is_empty() { continue; }
                let c: Vec<&str> = line.split(',').collect();
                if c.len() < 2 { continue; }
    ...
    fn build_loo_roc() -> LooRoc {
        let path = "models/loo_mlp_predictions.csv";
            ...
            egui::Frame::none()
                ...
                .show(ui, |ui| {
                    ...
                    ui.label(egui::RichText::new("ROC Curve & AUC (agregat lintas fold)")
                        .size(16.0).color(egui::Color32::from_rgb(0, 130, 110))
                        .family(egui::FontFamily::Name("PoppinsBold".into())));
                    ui.label(egui::RichText::new(format!(
...
/// OPSI 3 — Prediksi 1 FOLDER (≈20 CSV) memakai model MLP FULL hasil LOOO.
/// MLP menormalkan fitur secara internal (tidak perlu file norm terpisah).
#[allow(dead_code)]
fn run_folder_prediction_mlp(result_slot: std::sync::Arc<std::sync::Mutex<Option<String>>>) {
    ...
    let mut model = match CoffeeMLP::load("models/loo_mlp_full_model.json") {
        Ok(m) => m,
