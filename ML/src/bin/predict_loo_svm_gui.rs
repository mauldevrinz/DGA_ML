//! SVM LOOO Coffee Grade Predictor GUI
...
impl SVMPredictorApp {
    fn new(cc: &eframe::CreationContext<'_>) -> Self {
        ...
        if let Ok(model) = CoffeeSVM::load("models/loo_svm_full_model.json") {
        ...
        egui::TopBottomPanel::top("svm_predict_header")
            ...
            .show(ctx, |ui| {
                ui.vertical_centered(|ui| {
                    ui.add_space(12.0);
                    ui.label(
                        RichText::new("☕  SVM LOOO Coffee Grade Predictor")
                            .size(22.0)
                            .strong()
                            .color(Color32::WHITE),
                    );
...
fn run_prediction(
    ...
    for path in &csvs {
        ...
        if let Some((class, p_high, p_low)) = results.into_iter().next() {
...
fn main() -> Result<(), eframe::Error> {
    let options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            ...
            .with_title("SVM LOOO Coffee Grade Predictor"),
        ..Default::default()
    };

    eframe::run_native(
        "SVM LOOO LOOO Coffee Predictor",
        options,
