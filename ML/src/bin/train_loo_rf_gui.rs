//! RF LOOO Training GUI - Pure Rust Implementation (No Gnuplot)
...
struct TrainingGUI {
    ...
    // ── OPSI 3: hasil prediksi 1 folder (High/Low + confidence) ──
    #[allow(dead_code)]
...
#[allow(dead_code)]
    ...
    fn start_training(&mut self) {
        ...
        thread::spawn(move || {
            ...
            println!(
                "{}",
                "║    Random Forest Leave-One-Origin-Out Validation    ║"
                    .bold()
                    .cyan()
            );
            println!(
            ...
            let mut fold_rows: Vec<(String, f32, f32, usize, usize, usize, usize, usize, usize, usize, usize, f64)> = Vec::new(); // origin, test_acc, train_acc, test_cm[4], train_cm[4], secs
            ...
            println!("\n{}", "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━".cyan());
            println!("{}", " Phase 2/4: Random Forest on TSFRESH features".bold().cyan());
            println!("{}", "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━".cyan());

            // RF pakai fitur ter-normalisasi (split.train_features).
            let train_rf = &split.train_features;
            let val_rf = &split.test_features;
            ...
            println!("   Config: {} trees, max_depth={}, bootstrap={:.0}%",
                rf_config.n_trees, rf_config.max_depth, rf_config.bootstrap_fraction * 100.0);

            let training_start = Instant::now();
            let mut model = CoffeeRandomForest::new(rf_config);
            let _fit_result = match model.train(train_rf, &train_labels) {
                Ok(r) => r,
                Err(e) => { eprintln!("❌ Training error: {}", e); return; }
            };
            let training_time = training_start.elapsed();

            // Kurva: akurasi vs N-trees (RF tidak punya epoch/loss).
            // Evaluasi di SETIAP jumlah trees 1..n_total agar konsisten 100 titik.
            let n_total = model.n_trees();
            let checkpoints: Vec<usize> = (1..=n_total).collect();
            let mut final_metrics: Vec<TrainingMetrics> = Vec::new();
            for &n in &checkpoints {
                let ta = model.evaluate_at_n_trees_features(train_rf, &train_labels, n);
                let va = model.evaluate_at_n_trees_features(val_rf, &val_labels, n);
                final_metrics.push(TrainingMetrics {
                    train_loss: 0.0, train_accuracy: ta,
                    val_loss: 0.0, val_accuracy: va, is_final: true,
                });
            ...
            // Simpan kurva (N-trees, train_acc, val_acc) — epoch = N-trees
            {
                use std::io::Write;
                let curve_path = format!("models/loo_rf_curve_{}.csv", fold_idx);
            ...
            model.save("models/loo_rf_model.json").ok();

            } // ── akhir loop 12 fold ──

            // ── OPSI 3: Latih model FULL pakai SEMUA 12 origin (240 sampel) ──
            // LOOO di atas hanya untuk EVALUASI. Model untuk PREDICT dilatih ulang
            // di sini pada seluruh data, lalu disimpan bersama norm-stats-nya.
            {
                println!("\n🔧 Melatih model RF FULL (12 origin) untuk prediksi...");
                let full_norm = FeatureNormStats::fit(&dataset.features);
                let full_feat = full_norm.transform(&dataset.features);
                let full_cfg = RandomForestConfig {
                    n_trees: 100,
                    max_depth: 10,
                    min_samples_split: 4,
                    max_features: 0,
                ...
                let mut full_model = CoffeeRandomForest::new(full_cfg);
                match full_model.train(&full_feat, &dataset.labels) {
                    Ok(_) => {
                        std::fs::create_dir_all("models").ok();
                        full_model.save("models/loo_rf_full_model.json").ok();
                        full_norm.save("models/loo_rf_full_norm.json").ok();
                        println!("✅ Model RF FULL + norm tersimpan (loo_rf_full_model.json).");
                    }
                    Err(e) => eprintln!("❌ Gagal latih RF FULL: {}", e),
                }
            }
            ...
            {
                use std::io::Write;
                if let Ok(mut f) = std::fs::File::create("models/loo_rf_predictions.csv") {
                    let _ = writeln!(f, "p_high,label");
                    for (s, l) in all_scores.iter().zip(all_labels.iter()) {
                        let _ = writeln!(f, "{:.6},{}", s, l);
                    }
                }
                if let Ok(mut f) = std::fs::File::create("models/loo_rf_folds.csv") {
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("header")
            ...
            .show(ctx, |ui| {
                ui.vertical_centered(|ui| {
                    ui.add_space(15.0);
                    ui.label(egui::RichText::new("Leave-One-Origin-Out for Random Forest")
                        .size(28.0)
        ...
        egui::CentralPanel::default()
            ...
            .show(ctx, |ui| {
                ...
                if !self.training_complete {
                    ui.vertical_centered(|ui| {
                        ui.label(egui::RichText::new("Pilih 1 sumber kopi sebagai TEST — sisanya (11) jadi TRAIN")
                            .size(16.0)
            ...
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    ...
                    if pred_resp.clicked() {
                        launch_predict_binary("predict_loo_rf_gui");
                    }
...
impl TrainingGUI {
    fn save_pdf(&mut self) {
        ...
        if let Ok(content) = std::fs::read_to_string("models/loo_rf_folds.csv") {
        ...
        match generate_loo_summary_pdf("RF", &folds, &loo_roc, &agg_eval, total_secs, "testing_results") {
    ...
    fn build_agg_eval() -> EvaluationResults {
        ...
        if let Ok(content) = std::fs::read_to_string("models/loo_rf_predictions.csv") {
            for (li, line) in content.lines().enumerate() {
                if li == 0 || line.trim().is_empty() { continue; }
                let c: Vec<&str> = line.split(',').collect();
                if c.len() < 2 { continue; }
    ...
    fn build_loo_roc() -> LooRoc {
        let path = "models/loo_rf_predictions.csv";
            ...
            egui::Frame::none()
                ...
                .show(ui, |ui| {
                    ...
                    ui.label(egui::RichText::new("ROC Curve & AUC (agregat lintas fold)")
                        .size(16.0).color(egui::Color32::from_rgb(34, 139, 34))
                        .family(egui::FontFamily::Name("PoppinsBold".into())));
                    ui.label(egui::RichText::new(format!(
...
/// OPSI 3 — Prediksi 1 FOLDER (≈20 CSV) memakai model RF FULL hasil LOOO.
/// Tiap file diprediksi, lalu MAJORITY VOTE + rata-rata confidence kelas pemenang.
/// Output ditulis ke `result_slot` untuk ditampilkan di GUI.
#[allow(dead_code)]
fn run_folder_prediction_rf(result_slot: std::sync::Arc<std::sync::Mutex<Option<String>>>) {
    // Pilih folder
    ...
    // Muat model FULL + norm
    let model = match CoffeeRandomForest::load("models/loo_rf_full_model.json") {
        Ok(m) => m,
        Err(_) => {
    ...
    let norm = match FeatureNormStats::load("models/loo_rf_full_norm.json") {
        Ok(n) => n,
        Err(_) => {
            if let Ok(mut s) = result_slot.lock() {
                *s = Some("❌ Norm-stats belum ada. Jalankan START LOOO dulu.".to_string());
            }
            return;
        }
    };

    // Kumpulkan semua CSV di folder
    let mut csvs: Vec<std::path::PathBuf> = Vec::new();
    if let Ok(rd) = std::fs::read_dir(&folder) {
        for e in rd.flatten() {
            let p = e.path();
            if p.extension().map(|x| x.eq_ignore_ascii_case("csv")).unwrap_or(false) {
                csvs.push(p);
            }
        }
    }
    ...
    if csvs.is_empty() {
        if let Ok(mut s) = result_slot.lock() {
            *s = Some("❌ Tidak ada file CSV di folder itu.".to_string());
        }
        return;
    }
    ...
    let mut conf_high_sum = 0.0f32;
    let mut conf_low_sum = 0.0f32;
    let mut n_ok = 0usize;
    for path in &csvs {
        let pstr = path.to_string_lossy().to_string();
        let extracted = match extract_features_via_python(&pstr) {
            Ok(e) => e,
            Err(_) => continue, // lewati file yang gagal diekstrak
        };
        let feat_norm = norm.transform(&extracted.features);
        let probs = model.predict_proba_features(&feat_norm);
        let p_high = probs[[0, 0]];
        let p_low = probs[[0, 1]];
    ...
    if n_ok == 0 {
        if let Ok(mut s) = result_slot.lock() {
            *s = Some("❌ Semua file gagal diekstrak (cek Python/tsfresh).".to_string());
        }
        return;
    }

    // Majority vote + rata-rata confidence kelas pemenang
    ...
    let winner_votes = n_high.max(n_low);

    let msg = format!(
        "{}  —  {}/{} suara,  confidence {:.1}%",
        label, winner_votes, n_ok, conf * 100.0
    );
