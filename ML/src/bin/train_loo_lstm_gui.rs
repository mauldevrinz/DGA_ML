//! LSTM LOOO Training GUI - Pure Rust Implementation (No Gnuplot)
...
#[allow(dead_code)]
    ...
    fn start_training(&mut self) {
        ...
        thread::spawn(move || {
            ...
            println!(
                "{}",
                "║      Coffee Arabica LSTM Classifier           ║"
                    .bold()
                    .cyan()
            );
            println!(
                "{}",
                "║    LSTM Leave-One-Origin-Out Testing    ║"
                    .bold()
                    .cyan()
            );
            println!(
            ...
            if let Ok(rd) = std::fs::read_dir("models") {
                for entry in rd.flatten() {
                    ...
                    if name.starts_with("loo_lstm_curve_") && name.ends_with(".csv") {
            ...
            println!("\n{}", "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━".cyan());
            println!("{}", " Phase 2/4: LSTM on TSFRESH features".bold().cyan());
            println!("{}", "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━".cyan());

            // LSTM butuh (samples, channels=1, timesteps=n_features). Pakai fitur ter-normalisasi
            // (LSTM tidak normalisasi internal, jadi gunakan split.train_features yang sudah z-score).
            ...
            let val_lstm = to_lstm(&split.test_features);
            ...
            let lstm_config = LSTMConfig {
                ...
                stride: 1,
                clip_grad: 5.0,
                disable_early_stop: true,  // LOOO: latih epoch penuh
            };
            println!("   Config: hidden={}, lr={}, epochs={}, {} timesteps",
                lstm_config.hidden_size, lstm_config.learning_rate, lstm_config.n_epochs, n_features);

            let training_start = Instant::now();
            let mut model = CoffeeLSTM::new_with_input_size(lstm_config, 1);
            let live = tx.clone();
            let (_fit_result, accuracy_curve, loss_curve) = match model.fit_with_curve(
                &train_lstm,
                &train_labels,
                &val_lstm,
                &val_labels,
            ...
            // Bangun metrics_history dari kurva (untuk grafik GUI)
            let mut final_metrics: Vec<TrainingMetrics> = Vec::new();
            for i in 0..accuracy_curve.len() {
                let (_, tr_acc, va_acc) = accuracy_curve[i];
                let (_, tr_loss, va_loss) = loss_curve.get(i).cloned().unwrap_or((0, 0.0, 0.0));
                final_metrics.push(TrainingMetrics {
                    train_loss: tr_loss, train_accuracy: tr_acc,
                    val_loss: va_loss, val_accuracy: va_acc, is_final: true,
                });
            }
            ...
            // Simpan kurva per-epoch fold ini (untuk PDF detail)
            {
                use std::io::Write;
                let curve_path = format!("models/loo_lstm_curve_{}.csv", fold_idx);
                if let Ok(mut cf) = std::fs::File::create(&curve_path) {
                    ...
                    for (ep, m) in final_metrics.iter().enumerate() {
                        let _ = writeln!(cf, "{},{:.6},{:.6},{:.6},{:.6}",
                            ep + 1, m.train_accuracy, m.val_accuracy, m.train_loss, m.val_loss);
                    }
                }
            }
            ...
            // Prediksi (LSTM.predict → Array2 prob per-sampel)
            let train_probs = model.predict(&train_lstm);
            ...
            let val_probs = model.predict(&val_lstm);
            ...
            for i in 0..val_probs.shape()[0] {
                all_scores.push(val_probs.row(i)[1]); // P(high)
            ...
            std::fs::create_dir_all("models").ok();
            model.save("models/loo_lstm_model.json").ok();

            } // ── akhir loop 12 fold ──

            // ── OPSI 3: Latih LSTM FULL pakai SEMUA 12 origin (untuk prediksi) ──
            // LSTM butuh z-score eksternal lalu reshape ke (n,1,n_features).
            {
                println!("\n🔧 Melatih model LSTM FULL (12 origin) untuk prediksi...");
                ...
                match full_model.fit_with_curve(&full_3d, &dataset.labels, &full_3d, &dataset.labels, noop) {
                    Ok(_) => {
                        std::fs::create_dir_all("models").ok();
                        full_model.save("models/loo_lstm_full_model.json").ok();
                        full_norm.save("models/loo_lstm_full_norm.json").ok();
                        println!("✅ Model LSTM FULL + norm tersimpan (loo_lstm_full_model.json).");
                    }
                    Err(e) => eprintln!("❌ Gagal latih LSTM FULL: {}", e),
                }
            }
            ...
            {
                use std::io::Write;
                if let Ok(mut f) = std::fs::File::create("models/loo_lstm_predictions.csv") {
                    let _ = writeln!(f, "p_high,label");
                    for (s, l) in all_scores.iter().zip(all_labels.iter()) {
                        let _ = writeln!(f, "{:.6},{}", s, l);
                    }
                }
                if let Ok(mut f) = std::fs::File::create("models/loo_lstm_folds.csv") {
...
impl eframe::App for TrainingGUI {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("header")
            .exact_height(60.0)
            .frame(egui::Frame::none().fill(egui::Color32::from_rgb(100, 20, 180)))
            .show(ctx, |ui| {
                ui.vertical_centered(|ui| {
                    ui.add_space(15.0);
                    ui.label(egui::RichText::new("Leave-One-Origin-Out for LSTM")
                        .size(28.0)
            ...
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    ...
                    if pred_resp.clicked() {
                        launch_predict_binary("predict_loo_lstm_gui");
                    }
...
impl TrainingGUI {
    fn save_pdf(&mut self) {
        ...
        if let Ok(content) = std::fs::read_to_string("models/loo_lstm_folds.csv") {
        ...
        match generate_loo_summary_pdf("LSTM", &folds, &loo_roc, &agg_eval, total_secs, "testing_results") {
    ...
    fn build_agg_eval() -> EvaluationResults {
        ...
        if let Ok(content) = std::fs::read_to_string("models/loo_lstm_predictions.csv") {
            for (li, line) in content.lines().enumerate() {
                if li == 0 || line.trim().is_empty() { continue; }
                let c: Vec<&str> = line.split(',').collect();
                if c.len() < 2 { continue; }
    ...
    fn build_loo_roc() -> LooRoc {
        let path = "models/loo_lstm_predictions.csv";
            ...
            egui::Frame::none()
                ...
                .show(ui, |ui| {
                    ...
                    ui.label(egui::RichText::new("ROC Curve & AUC (agregat lintas fold)")
                        .size(16.0).color(egui::Color32::from_rgb(100, 20, 180))
                        .family(egui::FontFamily::Name("PoppinsBold".into())));
                    ui.label(egui::RichText::new(format!(
...
/// OPSI 3 — Prediksi 1 FOLDER (≈20 CSV) memakai model LSTM FULL hasil LOOO.
#[allow(dead_code)]
fn run_folder_prediction_lstm(result_slot: std::sync::Arc<std::sync::Mutex<Option<String>>>) {
    ...
    let model = match CoffeeLSTM::load("models/loo_lstm_full_model.json") {
        Ok(m) => m,
        Err(_) => { if let Ok(mut s)=result_slot.lock(){*s=Some("❌ Model belum ada. Jalankan START LOOO dulu.".to_string());} return; }
    };
    let norm = match FeatureNormStats::load("models/loo_lstm_full_norm.json") {
        Ok(n) => n,
        Err(_) => { if let Ok(mut s)=result_slot.lock(){*s=Some("❌ Norm-stats belum ada. Jalankan START LOOO dulu.".to_string());} return; }
    };
    ...
    csvs.sort();
    if csvs.is_empty() { if let Ok(mut s)=result_slot.lock(){*s=Some("❌ Tidak ada file CSV di folder itu.".to_string());} return; }
    ...
    for path in &csvs {
        ...
        let mut x3 = ndarray::Array3::<f32>::zeros((1, 1, nf));
        for j in 0..nf { x3[[0,0,j]] = feat_norm[[0,j]]; }
        let probs = model.predict(&x3);
        let p_high = probs[[0,1]];
        let p_low  = probs[[0,0]];
        if p_high >= p_low { n_high+=1; conf_high_sum+=p_high; } else { n_low+=1; conf_low_sum+=p_low; }
        n_ok+=1;
    }
    if n_ok==0 { if let Ok(mut s)=result_slot.lock(){*s=Some("❌ Semua file gagal diekstrak (cek Python/tsfresh).".to_string());} return; }
