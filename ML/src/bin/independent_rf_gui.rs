//! RF Cross Day Assessment GUI
//! Train: full RF model (all 12 origins, loo_rf_full_model.json).
//! Test : independent coffee batches in `data/Sample Ulang dan Baru/` (outside origins).
//! Metrik per-file (accuracy/precision/recall/F1/AUC + confusion matrix) + ringkasan
//! per-sampel (majority vote), lalu Assessment -> PDF.
...
impl App {
    fn new(cc: &eframe::CreationContext<'_>) -> Self {
        ...
        if Path::new("models/loo_rf_full_model.json").exists() { parts.push("model ready"); }
...
fn ensure_full_model() -> Result<(), String> {
    if Path::new("models/loo_rf_full_model.json").exists()
        && Path::new("models/loo_rf_full_norm.json").exists() {
        return Ok(());
    }
    println!("🔧 Model full belum ada — melatih RF FULL (12 origin)...");
    ...
    full_model.train(&full_feat, &dataset.labels)
        .map_err(|e| format!("Failed to train full RF model: {}", e))?;
    std::fs::create_dir_all("models").ok();
    full_model.save("models/loo_rf_full_model.json")
        .map_err(|e| format!("Failed to save model: {}", e))?;
    full_norm.save("models/loo_rf_full_norm.json")
        .map_err(|e| format!("Failed to save normalization: {}", e))?;
    println!("✅ Model RF FULL + norm tersimpan.");
    Ok(())
}
...
fn run_independent_eval() -> Result<EvalDone, String> {
    ...
    ensure_full_model()?;
    let norm = load_norm("models/loo_rf_full_norm.json")
        .map_err(|e| format!("Failed to load normalization: {}", e))?;
    let model = CoffeeRandomForest::load("models/loo_rf_full_model.json")
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("hdr").show(ctx, |ui| {
            ...
            ui.vertical_centered(|ui| {
                ui.label(RichText::new("☕  RF Cross Day Assessment")
                    .size(22.0).strong().color(Color32::from_rgb(120, 40, 180)));
                ui.label(RichText::new("Train: full model (12 origins)  -  Test: independent batches")
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                let start_btn = egui::Button::new(RichText::new("▶  START").size(15.0).color(Color32::WHITE))
                    .fill(Color32::from_rgb(120, 40, 180)).min_size(egui::vec2(150.0, 40.0));
                ...
                let assess_btn = egui::Button::new(RichText::new("📄  ASSESSMENT").size(15.0).color(Color32::WHITE))
                    .fill(Color32::from_rgb(90, 30, 140)).min_size(egui::vec2(170.0, 40.0));
...
fn main() -> eframe::Result<()> {
    println!("=== CROSS DAY RF (train full 12 origin, test cross-day/independent batches) ===");
    let opts = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            .with_inner_size([900.0, 680.0])
            .with_title("RF Cross Day Assessment"),
        ..Default::default()
    };
    eframe::run_native("RF Independent", opts, Box::new(|cc| Ok(Box::new(App::new(cc)))))
}