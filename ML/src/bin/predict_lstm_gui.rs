//! LSTM Coffee Grade Predictor GUI
//! Prediction interface using pre-trained LSTM model (lstm_model.json)

use coffee_classifier::ml::{CoffeeLSTM, FeatureNormStats, extract_features_via_python};
use eframe::egui;
...
impl LSTMPredictorApp {
    fn new(cc: &eframe::CreationContext<'_>) -> Self {
        ...
        if let Ok(stats) = load_normalization_stats("models/lstm_norm_stats.json") {
        ...
        if let Ok(model) = CoffeeLSTM::load("models/lstm_model.json") {
            println!("✅ LSTM model loaded");
            status_parts.push("✅ LSTM model");
            app.model = Some(model);
        } else {
            println!("⚠️ Failed to load LSTM model");
            status_parts.push("❌ LSTM model missing");
        }
...
impl eframe::App for LSTMPredictorApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // Header panel
        egui::TopBottomPanel::top("lstm_predict_header")
            .exact_height(70.0)
            .frame(egui::Frame::none().fill(Color32::from_rgb(100, 20, 180)))
            .show(ctx, |ui| {
                ui.vertical_centered(|ui| {
                    ui.add_space(12.0);
                    ui.label(
                        RichText::new("☕  LSTM Coffee Grade Predictor")
                            .size(22.0)
                            .strong()
                            .color(Color32::WHITE),
                    );
        ...
        // Status bar
        egui::TopBottomPanel::top("lstm_predict_status")
            .exact_height(28.0)
            .frame(egui::Frame::none().fill(Color32::from_rgb(248, 242, 255)))
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    ui.add_space(12.0);
                    ui.label(
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            egui::Frame::none()
                ...
                .stroke(egui::Stroke::new(1.0, Color32::from_rgb(180, 140, 230)))
                .show(ui, |ui| {
                    ui.label(
                        RichText::new("📂  Select CSV File")
                            .size(14.0)
                            .strong()
                            .color(Color32::from_rgb(100, 20, 180)),
                    );
                    ui.add_space(8.0);
                    ui.horizontal(|ui| {
                        ...
                        if ui
                            .add(
                                egui::Button::new(RichText::new("📁 Browse").size(13.0))
                                    .fill(Color32::from_rgb(100, 20, 180))
            ...
            ui.vertical_centered(|ui| {
                ...
                let bg = if is_loading {
                    ...
                    Color32::from_rgb(80, 10, 150)
                } else {
                    Color32::from_rgb(100, 20, 180)
                };
            ...
            match &self.state.clone() {
                PredictionState::Idle => {
                    ui.vertical_centered(|ui| {
                        ...
                        ui.add_space(8.0);
                        ui.label(
                            RichText::new("Model: LSTM  •  Classes: High Grade / Low Grade")
                                .size(12.0)
                                .color(Color32::from_rgb(130, 80, 180)),
                        );
                    });
                }
...
fn run_prediction(
    ...
    // Ekstrak fitur TSFRESH via Python (seragam dengan training LSTM baru)
    ...
    // Reshape (1, n) → (1, 1, n) untuk LSTM
    let mut data = ndarray::Array3::<f32>::zeros((1, 1, n_features));
    for j in 0..n_features { data[[0, 0, j]] = feat_norm[[0, j]]; }

    let probs = model.predict(&data);
    // Konvensi TSFRESH: kolom 1 = high, kolom 0 = low
    let low_grade_prob = probs[[0, 0]];
    let high_grade_prob = probs[[0, 1]];
...
fn main() -> Result<(), eframe::Error> {
    let options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            ...
            .with_title("LSTM Coffee Grade Predictor"),
        ..Default::default()
    };

    eframe::run_native(
        "LSTM Coffee Predictor",
        options,
        Box::new(|cc| Ok(Box::new(LSTMPredictorApp::new(cc)))),
    )
}