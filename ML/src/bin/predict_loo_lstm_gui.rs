//! LSTM LOOO Coffee Grade Predictor GUI
...
impl LSTMPredictorApp {
    fn new(cc: &eframe::CreationContext<'_>) -> Self {
        ...
        if let Ok(stats) = load_normalization_stats("models/loo_lstm_full_norm.json") {
        ...
        if let Ok(model) = CoffeeLSTM::load("models/loo_lstm_full_model.json") {
        ...
        egui::TopBottomPanel::top("lstm_predict_header")
            ...
            .show(ctx, |ui| {
                ui.vertical_centered(|ui| {
                    ui.add_space(12.0);
                    ui.label(
                        RichText::new("☕  LSTM LOOO Coffee Grade Predictor")
                            .size(22.0)
                            .strong()
                            .color(Color32::WHITE),
                    );
...
fn main() -> Result<(), eframe::Error> {
    let options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            ...
            .with_title("LSTM LOOO Coffee Grade Predictor"),
        ..Default::default()
    };

    eframe::run_native(
        "LSTM LOOO LOOO Coffee Predictor",
        options,
