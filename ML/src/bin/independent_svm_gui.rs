//! SVM Cross Day Assessment GUI
//! Train: full SVM model (all 12 origins, loo_svm_full_model.json).
...
impl App {
    fn new(cc: &eframe::CreationContext<'_>) -> Self {
        ...
        if Path::new("models/loo_svm_full_model.json").exists() { parts.push("Model ready"); }
...
fn ensure_full_model() -> Result<(), String> {
    if Path::new("models/loo_svm_full_model.json").exists() {
        return Ok(());
    }
    println!("🔧 Model full belum ada — melatih SVM FULL (12 origin)...");
    ...
    full_model.fit_with_curve_features(&dataset.features, &dataset.labels, &dataset.features, &dataset.labels, noop)
        .map_err(|e| format!("Gagal latih SVM FULL: {}", e))?;
    std::fs::create_dir_all("models").ok();
    full_model.save("models/loo_svm_full_model.json")
        .map_err(|e| format!("Gagal simpan model: {}", e))?;
    println!("✅ Model SVM FULL tersimpan.");
    Ok(())
}
...
fn run_independent_eval() -> Result<EvalDone, String> {
    ...
    ensure_full_model()?;
    let model = CoffeeSVM::load("models/loo_svm_full_model.json")
    ...
    for (name, true_label, dir) in &sample_dirs {
        ...
        for path in &csvs {
            ...
            let (p_high, p_low) = match results.into_iter().next() {
                Some((_c, ph, pl)) => (ph, pl),
                None => continue,
            };
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("hdr").show(ctx, |ui| {
            ...
            ui.vertical_centered(|ui| {
                ui.label(RichText::new("☕  SVM Cross Day Assessment")
...
fn main() -> eframe::Result<()> {
    println!("=== CROSS DAY SVM (train full 12 origin, test cross-day/independent batches) ===");
    let opts = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            .with_inner_size([900.0, 680.0])
            .with_title("SVM Cross Day Assessment"),
        ..Default::default()
    };
    eframe::run_native("SVM Independent", opts, Box::new(|cc| Ok(Box::new(App::new(cc)))))
}
