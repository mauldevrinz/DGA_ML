//! MLP Cross Day Assessment GUI
//! Train: full MLP model (all 12 origins, loo_mlp_full_model.json).
...
impl App {
    fn new(cc: &eframe::CreationContext<'_>) -> Self {
        ...
        if Path::new("models/loo_mlp_full_model.json").exists() { parts.push("Model ready"); }
...
fn ensure_full_model() -> Result<(), String> {
    if Path::new("models/loo_mlp_full_model.json").exists() {
        return Ok(());
    }
    println!("🔧 Model full belum ada — melatih MLP FULL (12 origin)...");
    ...
    full_model.fit_features(&dataset.features, &dataset.labels, &dataset.features, &dataset.labels, noop)
        .map_err(|e| format!("Gagal latih MLP FULL: {}", e))?;
    std::fs::create_dir_all("models").ok();
    full_model.save("models/loo_mlp_full_model.json")
        .map_err(|e| format!("Gagal simpan model: {}", e))?;
    println!("✅ Model MLP FULL tersimpan.");
    Ok(())
}
...
fn run_independent_eval() -> Result<EvalDone, String> {
    ...
    ensure_full_model()?;
    let mut model = CoffeeMLP::load("models/loo_mlp_full_model.json")
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("hdr").show(ctx, |ui| {
            ...
            ui.vertical_centered(|ui| {
                ui.label(RichText::new("☕  MLP Cross Day Assessment")
...
fn main() -> eframe::Result<()> {
    println!("=== CROSS DAY MLP (train full 12 origin, test cross-day/independent batches) ===");
    let opts = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            .with_inner_size([900.0, 680.0])
            .with_title("MLP Cross Day Assessment"),
        ..Default::default()
    };
    eframe::run_native("MLP Independent", opts, Box::new(|cc| Ok(Box::new(App::new(cc)))))
}
