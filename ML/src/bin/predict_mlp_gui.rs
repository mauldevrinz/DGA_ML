//! MLP Coffee Grade Predictor GUI
...
impl MLPPredictorApp {
    fn new(cc: &eframe::CreationContext<'_>) -> Self {
        ...
        if let Ok(model) = CoffeeMLP::load("models/mlp_model.json") {
        ...
        egui::TopBottomPanel::top("mlp_predict_header")
            ...
            .show(ctx, |ui| {
                ui.vertical_centered(|ui| {
                    ui.add_space(12.0);
                    ui.label(
                        RichText::new("☕  MLP Coffee Grade Predictor")
                            .size(22.0)
                            .strong()
                            .color(Color32::WHITE),
                    );
...
fn run_prediction(
    ...
    // Ekstrak fitur TSFRESH via Python (MLP normalisasi internal sendiri)
    let extracted = extract_features_via_python(file_path)
        .map_err(|e| format!("Ekstraksi TSFRESH gagal: {}", e))?;

    let (high_grade_prob, low_grade_prob) = model.predict_proba_features(&extracted.features);
...
fn main() -> Result<(), eframe::Error> {
    let options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            ...
            .with_title("MLP Coffee Grade Predictor"),
        ..Default::default()
    };

    eframe::run_native(
        "MLP Coffee Predictor",
        options,
