    ...
    fn start_training(&mut self) {
        ...
        thread::spawn(move || {
            ...
            let training_config = TrainingConfig {
                ...
                num_epochs: 100,
                weight_decay: 0.0001,
                patience: 100000,        // LOOO: matikan early stopping (epoch penuh)
                min_delta: 0.0,
                max_overfitting_gap: 1.0, // jangan stop karena gap
            ...
            if let Ok(rd) = std::fs::read_dir("models") {
                for entry in rd.flatten() {
                    ...
                    if name.starts_with("loo_cnn_curve_") && name.ends_with(".csv") {
            ...
            let val_cnn = to_cnn(&split.test_features);
            ...
            let trainer = Trainer::new(training_config.clone());
            ...
            {
                use std::io::Write;
                let curve_path = format!("models/loo_cnn_curve_{}.csv", fold_idx);
                if let Ok(mut cf) = std::fs::File::create(&curve_path) {
                    let _ = writeln!(cf, "epoch,train_acc,val_acc,train_loss,val_loss");
                    let finals: Vec<&TrainingMetrics> = final_metrics.iter().filter(|m| m.is_final).collect();
                    for (ep, m) in finals.iter().enumerate() {
            ...
            std::fs::create_dir_all("models").ok();
            model.save("models/loo_cnn_model.json").ok();

            } // ── akhir loop 12 fold ──

            // ── OPSI 3: Latih CNN FULL pakai SEMUA 12 origin (untuk prediksi) ──
            // CNN (TSFRESH 1-channel): z-score eksternal -> reshape (n,1,nf) -> Trainer.
            {
                println!("\n🔧 Melatih model CNN FULL (12 origin) untuk prediksi...");
                ...
                std::fs::create_dir_all("models").ok();
                full_model.save("models/loo_cnn_full_model.json").ok();
                full_norm.save("models/loo_cnn_full_norm.json").ok();
                println!("✅ Model CNN FULL + norm tersimpan (loo_cnn_full_model.json).");
            }

            // ── Tulis semua prediksi + metrik fold ke file untuk PDF agregat ──
            {
                use std::io::Write;
                if let Ok(mut f) = std::fs::File::create("models/loo_cnn_predictions.csv") {
                    let _ = writeln!(f, "p_high,label");
                    for (s, l) in all_scores.iter().zip(all_labels.iter()) {
                        let _ = writeln!(f, "{:.6},{}", s, l);
                    }
                }
                if let Ok(mut f) = std::fs::File::create("models/loo_cnn_folds.csv") {
        ...
        egui::TopBottomPanel::top("header")
            ...
            .show(ctx, |ui| {
                ui.vertical_centered(|ui| {
                    ui.add_space(15.0);
                    ui.label(egui::RichText::new("Leave-One-Origin-Out for CNN")
                        .size(28.0)
            ...
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    ...
                    if pred_resp.clicked() {
                        launch_predict_binary("predict_loo_cnn_gui");
                    }
...
impl TrainingGUI {
    fn save_pdf(&mut self) {
        ...
        if let Ok(content) = std::fs::read_to_string("models/loo_cnn_folds.csv") {
        ...
        match generate_loo_summary_pdf("CNN", &folds, &loo_roc, &agg_eval, total_secs, "testing_results") {
    ...
    fn build_agg_eval() -> EvaluationResults {
        ...
        if let Ok(content) = std::fs::read_to_string("models/loo_cnn_predictions.csv") {
            for (li, line) in content.lines().enumerate() {
                if li == 0 || line.trim().is_empty() { continue; }
                let c: Vec<&str> = line.split(',').collect();
                if c.len() < 2 { continue; }
    ...
    fn build_loo_roc() -> LooRoc {
        let path = "models/loo_cnn_predictions.csv";
            ...
            egui::Frame::none()
                ...
                .show(ui, |ui| {
                    ...
                    ui.label(egui::RichText::new("ROC Curve & AUC (agregat lintas fold)")
                        .size(16.0).color(egui::Color32::from_rgb(15, 92, 112))
                        .family(egui::FontFamily::Name("PoppinsBold".into())));
                    ui.label(egui::RichText::new(format!(
...
/// OPSI 3 — Prediksi 1 FOLDER (≈20 CSV) memakai model CNN FULL hasil LOOO.
#[allow(dead_code)]
fn run_folder_prediction_cnn(result_slot: std::sync::Arc<std::sync::Mutex<Option<String>>>) {
    ...
    let mut model = match CoffeeCNN::load("models/loo_cnn_full_model.json") {
        Ok(m) => m,
        Err(_) => { if let Ok(mut s)=result_slot.lock(){*s=Some("❌ Model belum ada. Jalankan START LOOO dulu.".to_string());} return; }
    };
    let norm = match FeatureNormStats::load("models/loo_cnn_full_norm.json") {
        Ok(n) => n,
