...
impl CoffeePredictorApp {
    fn new(cc: &eframe::CreationContext<'_>) -> Self {
        ...
        if let Ok(stats) = load_normalization_stats("models/loo_cnn_full_norm.json") {
        ...
        // Load model
        if let Ok(model) = CoffeeCNN::load("models/loo_cnn_full_model.json") {
...
impl eframe::App for CoffeePredictorApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ui.heading(RichText::new("☕ CNN LOOO Coffee Grade Predictor").size(24.0).strong());
            ui.add_space(10.0);

            // File selection
            ui.horizontal(|ui| {
                ui.label("Folder (20 file):");
            ...
            match &self.state {
                PredictionState::Idle => {
                    ui.vertical_centered(|ui| {
                        ui.label(RichText::new("Pilih folder berisi 20 file and click Predict").size(14.0).color(Color32::GRAY));
                    });
                }
...
fn main() -> Result<(), eframe::Error> {
    let options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            ...
            .with_title("CNN LOOO Coffee Grade Predictor"),
        ..Default::default()
    };
