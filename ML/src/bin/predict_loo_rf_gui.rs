...
impl RFPredictorApp {
    fn new(cc: &eframe::CreationContext<'_>) -> Self {
        ...
        if let Ok(stats) = load_normalization_stats("models/loo_rf_full_norm.json") {
        ...
        if let Ok(model) = CoffeeRandomForest::load("models/loo_rf_full_model.json") {
        ...
        egui::TopBottomPanel::top("rf_predict_header")
            ...
            .show(ctx, |ui| {
                ui.vertical_centered(|ui| {
                    ui.add_space(12.0);
                    ui.label(
                        RichText::new("☕  Random Forest — LOOO Folder Predictor")
                            .size(22.0)
                            .strong()
                            .color(Color32::WHITE),
                    );
                    ui.add_space(4.0);
                    ui.label(
                        RichText::new("Pilih 1 folder (≈20 file) → 1 hasil (majority vote)")
                            .size(12.0)
            ...
            match &self.state.clone() {
                PredictionState::Idle => {
                    ui.vertical_centered(|ui| {
                        ui.add_space(40.0);
                        ui.label(
                            RichText::new("📊  Pilih folder berisi 20 file lalu klik Predict")
                                .size(15.0)
                                .color(Color32::GRAY),
                        );
                        ui.add_space(8.0);
...
fn run_prediction(
    ...
    // Kumpulkan semua CSV dalam folder
    let mut csvs: Vec<std::path::PathBuf> = Vec::new();
    let rd = std::fs::read_dir(folder_path)
        .map_err(|e| format!("Gagal baca folder: {}", e))?;
    for e in rd.flatten() {
        let p = e.path();
    ...
    csvs.sort();
    if csvs.is_empty() {
        return Err("Tidak ada file CSV di folder itu.".to_string());
    }

    // Prediksi tiap file → kumpulkan vote + confidence
    ...
    for path in &csvs {
        ...
        let extracted = match extract_features_via_python(&pstr) {
            Ok(e) => e,
            Err(_) => continue, // lewati file gagal
        };
    ...
    if n_ok == 0 {
        return Err("Semua file gagal diekstrak (cek Python/tsfresh).".to_string());
    }
...
fn main() -> Result<(), eframe::Error> {
    let options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            ...
            .with_title("RF LOOO Coffee Grade Predictor"),
        ..Default::default()
    };
